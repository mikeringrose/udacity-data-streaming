...
redisServerDF = redisServerDF.selectExpr(
...
zSetEncodedEntriesDF = spark.sql(
...
# |[7B 22 73 74 61 7...|
# +--------------------+
zSetDecodedEntriesDF = zSetEncodedEntriesDF \
    .withColumn("customer", unbase64(zSetEncodedEntriesDF.encodedCustomer).cast("string"))
...
customerDF = zSetDecodedEntriesDF \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING
stediEventsDF = stediEventsDF.selectExpr(
    "CAST(key AS STRING) key",
    "CAST(value AS STRING) value"
)
...
customerRiskJoinDF = emailAndBirthYearStreamingDF \
    .join(customerRiskStreamingDF, expr("email = customer"))
